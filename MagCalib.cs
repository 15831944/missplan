...
namespace MissionPlanner
{
    public class MagCalib
    {
        ...
        static void prd_DoWork(IProgressReporterDialogue sender)
        {
            ...
            {
                MainV2.speechEngine.SpeakAsync("Kalibrasi kompas selesai");
            }
            else
            {
                Console.Beep();
            ...
            {
                sender.doWorkArgs.ErrorMessage = "Nilai asli kompas jelek. Silahkan cek magnetic interferance.";
                ans = null;
                ans2 = null;
                return;
            }
            ...
            {
                sender.doWorkArgs.ErrorMessage = "Log tidak memiliki data yang cukup";
                ans = null;
                ans2 = null;
                return;
            }
        ...
        public static double[] getOffsets(string fn, int throttleThreshold = 0)
        {
            ...
            using (MAVLinkInterface mine = new MAVLinkInterface())
            {
                ...
                {
                    log.Debug(ex.ToString());
                    CustomMessageBox.Show("Log tidak bisa dibuka, apakah anda masih tersambung?");
                    return new double[] {0};
                }
            ...
            if (data.Count < 10)
            {
                CustomMessageBox.Show("Log tidak memiliki cukup data");
