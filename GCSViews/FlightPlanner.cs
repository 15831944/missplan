...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        public void setfromMap(double lat, double lng, int alt, double p1 = 0)
        {
            ...
            {
                CustomMessageBox.Show("Terdeteksi masukan yang salah.\n" + ex.Message, Strings.ERROR);
            }
                ...
                Commands.Columns[Alt.Index].HeaderText.Equals(cmdParamNames["WAYPOINT"][6] /*"Alt"*/))
            {
...

                {
                    ...
                    if (pass == false)
                    {
                        CustomMessageBox.Show("Anda harus memiliki a home altitude");
                        string homealt = "100";
                    ...
                    {
                        CustomMessageBox.Show("Default alt anda tidak valid");
                        return;
                    }
        ...
        public void BUT_read_Click(object sender, EventArgs e)
        {
            if (Commands.Rows.Count > 0)
            {
                ...
                {
                    if (
                        CustomMessageBox.Show("Ini akan menghapus seluruh misi perencanaan anda, Lanjutkan?", "Confirm",
        ...
        void getWPs(IProgressReporterDialogue sender)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("Sambungkan dahulu!");
                }
        ...
        public void BUT_write_Click(object sender, EventArgs e)
        {
            if ((altmode) CMB_altmode.SelectedValue == altmode.Absolute)
            {
                if ((int)DialogResult.No ==
                    CustomMessageBox.Show("Absolute Alt telah dipilih, apakah anda menentukan?", "Alt Mode", MessageBoxButtons.YesNo))
                {
            ...
            {
                CustomMessageBox.Show("Home location anda tidak valid", Strings.ERROR);
                return;
            }
            ...
            for (int a = 0; a < Commands.Rows.Count - 0; a++)
            {
                for (int b = 0; b < Commands.ColumnCount - 0; b++)
                {
                    ...
                    if (b >= 1 && b <= 7)
                    {
                        ...
                        {
                            CustomMessageBox.Show("Terdapat errors pada misi anda");
                            return;
                        }
                    }
        ...
        void saveWPsFast(IProgressReporterDialogue sender)
        {
            ...
            catch
            {
                ...
                throw new Exception("Home location anda tidak valid");
            }
        ...
        public void readQGC110wpfile(string file, bool append = false)
        {
            ...
            {
                CustomMessageBox.Show("Tidak bisa membuka file! " + ex);
            }
        }
        ...
        double calcpolygonarea(List<PointLatLng> polygon)
        {
            ...
            if (polygon.Count == 0)
            {
                CustomMessageBox.Show("Silahkan tentukan polygon!");
                return 0;
            }
            ...
            try // when dragging item can sometimes be null
            {
                ...
                if (ModifierKeys == Keys.Control)
                {
                    try
                    {
                        ...
                        log.Info("Tambah penanda untuk group");
                    }
                    catch (Exception ex)
                    {
                        log.Error(ex);
        ...
        */
        ...
        private void comboBoxMapType_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            {
                log.Error(ex);
                CustomMessageBox.Show("Gagal merubah Map. coba zoom out terlebih dahulu.");
            }
        }
        ...
        {
            if (!sethome)
                CustomMessageBox.Show("Klik pada map untuk menentukan Home ");
            sethome = true;
        ...
        private void label4_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            {
                CustomMessageBox.Show(
                    "Jika anda berada di lapangan, sambungkan ke APM anda dan tunggu sampai GPS terkunci. Kemudian klik 'Home Location' link untuk set lokasi anda sebagai home");
            }
        }
        ...
        private void deleteWPToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (CurentRectMarker != null)
            {
                if (int.TryParse(CurentRectMarker.InnerMarker.Tag.ToString(), out no))
                {
                    ...
                    {
                        log.Error(ex);
                        CustomMessageBox.Show("error selecting wp, coba lagi.");
                    }
                }
                else if (int.TryParse(CurentRectMarker.InnerMarker.Tag.ToString().Replace("grid", ""), out no))
                {
                    ...
                    {
                        log.Error(ex);
                        CustomMessageBox.Show("Remove point Failed. coba lagi.");
                    }
                }
            }
        ...
        private void GeoFenceuploadToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("Tidak Didukung");
                return;
            }

            if (drawnpolygon == null)
            {
                CustomMessageBox.Show("Tidak ada polygon untuk diunggah");
                return;
            }

            if (geofenceoverlay.Markers.Count == 0)
            {
                CustomMessageBox.Show("Lokasi kembali belum ditentukan");
                return;
            }

            if (drawnpolygon.Points.Count == 0)
            {
                CustomMessageBox.Show("polygon belum digambar");
                return;
            }
            ...
            {
                CustomMessageBox.Show("Lokasi kembali anda berada diluar polygon");
                return;
            }
        ...
        public void GeoFencedownloadToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("tidak ada yang diunduh");
                return;
            }
        ...
        private void saveToFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (geofenceoverlay.Markers.Count == 0)
            {
                CustomMessageBox.Show("Silahkan tentukan lokasi kembali.");
                return;
            }
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        private void but_writewpfast_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("Lokasi home anda tidak valid", Strings.ERROR);
                return;
            }
